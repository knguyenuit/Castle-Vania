...
CUpStairLeft::CUpStairLeft()
{
	this->Init();
}

CUpStairLeft::CUpStairLeft(Vector2 pos)
{
	this->Init();
	this->m_Pos = pos;
	this->m_typeHideObject = HideObject_TYPE::UpStairLeft;
}
...
void CUpStairLeft::Update(float deltaTime)
{
...
bool CUpStairLeft::SimonMoveOnStair(float deltaTime)
{
...
bool CUpStairLeft::SimonMoveToStair(float deltaTime)
{
	//Init Animation
...
bool CUpStairLeft::SimonMoveOutStair(float deltaTime)
{
	if (this->simon->MoveTo(this->simon->currStairPos + Vector2(0, 16), deltaTime))
	{
