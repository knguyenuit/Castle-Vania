...
CDownStairRight::CDownStairRight()
{
	this->Init();
}

CDownStairRight::CDownStairRight(Vector2 pos)
{
	this->Init();
	this->m_Pos = pos;
	this->m_typeHideObject = HideObject_TYPE::DownStairRight;
}
...
void CDownStairRight::Update(float deltaTime)
{
	...
	if (this->simon->currStairPos == this->m_Pos) // chi chay khi con khi con simon da dung trung no
	{
		...
		if (this->isSimonCollision == true)
		{
			...
			else // Nguoc lai neu dang khong nhan phim Down
			{
				if (this->simon->isArrowKeyUp == true) // Va nhan nut Up
				{
					if (this->simon->isOnStair == true) // Va con simon dang o tren cau thang 
					{
						...
						this->simon->isCancelStairMove = false;
						if (this->simon->m_Pos.x - this->simon->currStairPos.x <= 16) //Fix bug: dang o buoc thu 2 da len cau thang
						{
							this->simon->prepareOutStair = true; //Thi gan trang thai con simon la chuan bi ra khoi cau thang
						}

					}
					else //Nguoc lai : Neu nhan phim Up khi dang va cham voi voi cuc DownStairLeft nhung khong o trang thai tren cau thang
					{
						return; //Thoat
					}
				} //End press Up
...
bool CDownStairRight::SimonMoveOnStair(float deltaTime)
{
...
bool CDownStairRight::SimonMoveToStair(float deltaTime)
{
	//Init Animation
...
bool CDownStairRight::SimonMoveOutStair(float deltaTime)
{
	if (this->simon->MoveTo(this->simon->currStairPos + Vector2(-16, 16), deltaTime))
	{
		this->simon->m_Dir = Direction::LEFT;
		this->ResetSimonStair();
		return true;
	}
	else
